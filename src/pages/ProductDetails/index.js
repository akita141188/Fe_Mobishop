    ...
    return createCommentProduct(id, commentData)
      .then(({ data }) => {
        if (data.status == "success") {
          setInputComment({});
          getComments(id);
          setNotification("Bình luận của bạn đã được gửi thành công, hiện đang chờ phê duyệt!");
        }
      });
  }
      ...
      <div id="product">
        <div id="product-head" className="row">
          <div id="product-img" className="col-lg-6 col-md-6 col-sm-12">
            <img src={getImageProduct(product.thumbnails)} alt="Product Thumbnail" />
          </div>
          ...
          <div className="col-lg-12 col-md-12 col-sm-12">
            {
              comments?.map((comment, index) =>
                ...
                <div className="comment-item">
                  <ul>
                    <li>
                      <b>{`${index + 1}- ${comment.email}`}</b>
                    </li>
                    <li>{moment(comment.createdAt).locale("vi").format("LL LTS")}</li>
                    <li>
                      {comment.body}
                    </li>
                  </ul>
